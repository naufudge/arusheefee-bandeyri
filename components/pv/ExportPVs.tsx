...
const ExportPVs: React.FC<ExportPVsProps> = ({ year }) => {
  ...
  // Fetch as a blob (rather than an <a download> that fires and forgets)
  // so we have an explicit await point to drive the loading state.
  const handleExportClick = async () => {
    if (busy) return;
    setBusy(true);
    try {
      const res = await fetch(`/api/pv/export/${year}`);
      if (!res.ok) {
